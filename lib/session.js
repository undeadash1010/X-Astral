...
class Session {
  constructor() {
    ...
    try { await fs.rm('./session', { recursive: true, force: true });
      ...
      if (trimmedId.length > 20) {
        ...
        if (sessionData['creds.json']) {
          for (const [file, data] of Object.entries(sessionData)) {
            await this.writeFile(`./session/${file}`, data);
          }
        } else {
          await this.writeFile('./session/creds.json', sessionData);
        }
      } else {
        const paste = await this.pastebin.getPaste(trimmedId);
        if (!paste) {
          throw new Error('Invalid session ID');
        } await this.writeFile('./session/creds.json', paste.toString());
      }
    } catch (err) {
