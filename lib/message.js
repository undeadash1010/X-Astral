const { downloadContentFromMessage, jidDecode, getContentType } = require("baileys");
...
async function downloadMedia(msg, filePath) {
  try {
  ...
  if (msg.message) {
    ...
    try {
      ...
      if (quotedInfo && quotedInfo.quotedMessage) {
        ...
        msg.quoted.download = (filePath) => downloadMedia(msg.quoted.message, filePath);
      }
    } catch (err) {
    ...
    msg.download = (filePath) => downloadMedia(msg.message, filePath);
    sock.client = msg;
  }
  return msg;
}

module.exports = { serialize, downloadMedia, decodeJid };
